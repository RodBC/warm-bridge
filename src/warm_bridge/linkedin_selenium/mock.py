"""Offline fixtures when WARM_BRIDGE_SELENIUM_MOCK=1 — never invents beyond the fixture."""
...
def mock_mutuals(*, empty: bool = False, target_name: str = "Sabrina Coelho Godoy") -> list[dict[str, str]]:
    """2–3 fictional mutuals for eval/CI. Empty list when empty=True."""
    if empty:
        return []
    # Stable ids for eval assertions
    return [
        {
            "id": "li_mock_ana",
            "name": "Ana Ribeiro",
...
def mock_enrich(contact: dict[str, Any]) -> dict[str, Any]:
    """Optional enrich stub — only fills blank title/company from fixture patterns."""
    out = dict(contact)
