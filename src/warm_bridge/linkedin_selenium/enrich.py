...
# LinkedIn profile photo selectors (DOM drifts — try several).
_AVATAR_SELECTORS = [
...
def extract_avatar_url(driver: Any) -> str:
    """Return first usable LinkedIn CDN photo URL on the current page, or ''."""
    for sel in _AVATAR_SELECTORS:
        try:
            imgs = driver.find_elements("css selector", sel)
        ...
        for img in imgs:
            ...
            if "data:image" in src:
                continue
            # Prefer real profile photos over ghost/spacer assets
...
def _split_headline(text: str) -> tuple[str, str]:
    """Best-effort title/company from a LinkedIn headline string."""
    raw = (text or "").strip()
...
def extract_headline(driver: Any) -> str:
...
def extract_company(driver: Any) -> str:
    selectors = [
...
def enrich_profile_fields(driver: Any, row: dict[str, Any]) -> dict[str, Any]:
    """Fill missing title/company/avatar from the current profile page."""
    out = dict(row)
    headline = extract_headline(driver)
    if headline:
    ...
    if not out.get("company"):
        company = extract_company(driver)
        if company:
            out["company"] = company

    if not out.get("avatar_url") and not out.get("photo"):
        src = extract_avatar_url(driver)
        if src:
            out["avatar_url"] = src
            out["photo"] = src
...
def fetch_profile_snapshot(
    ...
    *,
    ...
    """Navigate to a profile; return avatar_url, title, company, headline (observed only)."""
    ...
    try:
        ...
        row = enrich_profile_fields(driver, {})
...
def fetch_profile_avatar(driver: Any, profile_url: str, *, rate: RateLimit | None = None) -> str:
    """Navigate to a profile and return avatar CDN URL (or '')."""
    return fetch_profile_snapshot(driver, profile_url, rate=rate).get("avatar_url") or ""
...
def enrich_contacts(
    ...
    *,
    ...
    """Visit up to `cap` profile pages; fill missing title/company/avatar. Never invents."""
    ...
    for i, c in enumerate(contacts):
        ...
        try:
            driver.get(url)
            polite_sleep(rate.between_nav_s)
            row = enrich_profile_fields(driver, row)
            polite_sleep(rate.after_action_s)
        except Exception:  # noqa: BLE001
            pass
        out.append(row)
